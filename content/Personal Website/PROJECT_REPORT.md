...
**최종 수정일**: 2026년 2월 26일 (모바일 학습 레이아웃 최적화 및 백그라운드 재생 기능 도입 완료)
**대상 도메인**: `chaeyul.uk`
...
- **로컬 전처리 자동화**: 서버 업로드 전 이미지 최적화(WebP 변환, 리사이징, 메타데이터 추출)를 위한 Python 전처리 스크립트(`preprocess_images.py`) 개발.
...
242: 
243: ### ⑪ 모바일 학습 환경 최적화 및 백그라운드 재생 [NEW - 02.26]
244: - **모바일 전용 레이아웃**: 자막 영역이 화면을 과도하게 차지하거나 하단 리스트를 가리지 않도록 모바일에서 자막 높이를 동적으로 조절하고 Sticky 설정을 최적화.
245: - **백그라운드 연속 재생 (MediaSession)**: 화면을 잠그거나 브라우저를 백그라운드로 전환해도 재생이 멈추지 않도록 `MediaSession` API 및 무음 오디오 유지(Silent Heartbeat) 기술 적용.
246: - **잠금 화면 제어**: 잠금 화면 및 알림창에서 재생/일시정지 및 구간 이동이 가능한 미디어 컨트롤 센터 기능 통합.

<img src="mobile_optimized_study.png" width="200" alt="모바일 학습 및 백그라운드 재생">
