...
timeline
    ...
    업그레이드 7 : Fail2ban 2중 포트 방어 & 직접 IP 스캐너 차단 방화벽
...
- **구현 내용**:
  - Kubernetes `hostPath` 볼륨 마운트를 적용하여 `/home/aiadmin01/edge-ai-engine/token_audit_logs.json` 파일을 두 Pod가 원자적으로 공유하도록 설정.
...
### 🚀 Upgrade 7: Fail2ban 2중 포트 보호 & 웹 직접 IP 차단 방화벽
- **도입 배경**: 엣지 서버 노드가 외부에 공개되었을 때 유입되는 불특정 다수의 침입 차단.
- **구현 내용**:
  - `setup_fail2ban.sh` 스크립트를 배포하여 22022 및 443 포트에 대한 SSH 침입을 방어(5회 실패 시 24시간 IP 밴).
